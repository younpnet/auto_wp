...
class WordPressAutoPoster:
    ...
    def clean_content(self, content):
        ...
        # 2. 하이퍼링크 내 도메인 중복 및 경로 파편 강제 교정
        def repair_links(match):
            url = match.group(1)
            # URL 내부에 https:// 가 다시 나오면 가장 마지막 주소만 선택
...
- 3,000자 이상의 압도적인 정보량과 실질적인 도움을 주는 조언을 포함하세요."""
        ...
        # [핵심 단계 1] AI가 혹시나 토큰 앞에 도메인을 붙였을 경우를 대비해 사전 정제
        final_content = post_data['content']
        for token in links_mapping.keys():
            # <a href="https://site.com/##TOKEN##"> 형태를 <a href="##TOKEN##">로 강제 교정
            final_content = re.sub(rf'href="https?://[^"]*/?{re.escape(token)}"', f'href="{token}"', final_content)

        # [핵심 단계 2] 토큰을 실제 원본 URL로 1:1 치환 (무결성 보장)
        for token, real_url in links_mapping.items():
            final_content = final_content.replace(token, real_url)
